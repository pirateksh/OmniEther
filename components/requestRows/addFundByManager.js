...
class RequestRow extends Component {
	...
	onApprove = async () => {
		...
		try {
			await fund.methods.approvefundRequestByManager(this.props.id)
				.send({
					from: accounts[0]
				});

			Router.pushRoute('getFundRequestsByManager', { contractAddress: this.props.address});
	...
	onFinalize = async () => {
		...
		try{
			...
			await fund.methods.finalizefundRequestByManager(this.props.id).send({
				from: accounts[0]
			});
